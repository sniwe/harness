...
test('benchmark adapters require explicit real verifier and source paths', () => {
  ...
  assert.equal(resolveAppBenchmarkAdapter({ projectRoot: root, verifier: 'verify.mjs', source: '987 source.mp3' }).args[0], verifier); assert.throws(() => resolveAppBenchmarkAdapter({ projectRoot: root, verifier: 'verify.mjs', source: 'other.mp3' }), /source_invalid/);
