...
test('app benchmark adapter verifies declared source revision', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'adapter-source-')); const verifier = path.join(root, 'verify.mjs'); const source = path.join(root, '987 source.mp3'); fs.writeFileSync(verifier, ''); fs.writeFileSync(source, 'source'); const revision = 'sha256:' + crypto.createHash('sha256').update('source').digest('hex');
  const adapter = resolveAppBenchmarkAdapter({ projectRoot: root, verifier: 'verify.mjs', source: '987 source.mp3', sourceAudioRevision: revision });
  assert.equal(adapter.sourceAudioRevision, revision);
});
