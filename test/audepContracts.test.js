...
test('warm benchmark cohort requires pinned comparable runs and reports median/range', () => {
  ...
  assert.equal(evaluateWarmBenchmarkCohort(runs.map((run, index) => index === 1 ? { ...run, configurationDigest: 'e'.repeat(64) } : run)).reason, 'warm_cohort_identity_mismatch'); assert.equal(evaluateWarmBenchmarkCohort(runs.map((run, index) => index === 0 ? { ...run, metrics: { ...run.metrics, committedRealtime: 0.49 } } : run)).verdict, 'failed'); assert.equal(evaluateWarmBenchmarkCohort(runs.map((run, index) => index === 1 ? { ...run, runId: 'other' } : run)).reason, 'warm_cohort_identity_mismatch'); assert.equal(evaluateWarmBenchmarkCohort([runs[0], { ...runs[1], benchmarkId: runs[0].benchmarkId }, runs[2]]).field, 'benchmarkId');
});
