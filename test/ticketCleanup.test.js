...
test('cleanup journals intent and retires source plus receipt with bounded removal', async () => {
  ...
  const result = await createTicketCleanup({ client, journal, sleep: async () => {} }).cleanupPair('source', 'receipt'); assert.deepEqual(result, { sourceDeleted: true, receiptDeleted: true }); assert.deepEqual(calls.map((call) => call.patch.action), ['begin_delete', 'begin_delete']); assert.deepEqual(journal.entries.map((entry) => entry.event), ['cleanup_intent', 'mutation_intent', 'mutation_result', 'source_cleanup_complete', 'mutation_intent', 'mutation_result', 'ticket_retired']);
});
