...
test('rehearsal cannot report complete when bilateral final acceptance is blocked', async () => {
  const manifest = { schemaVersion: 1, runId: 'rehearsal-acceptance', planDigest: 'f'.repeat(64), steps: [{ stepId: 'A0', owner: 'app', dependsOn: [], commandProfile: 'test', verifierProfile: 'focused', requiredOutputTypes: ['acceptance'] }] };
  const benchmark = { schemaVersion: 1, benchmarkId: 'b', runId: 'r', requestArtifactId: 'a'.repeat(64), source: { basename: '987.mp3', durationMs: 1 }, job: { remoteJobId: 'j' }, runtime: { appGeneration: 'a', qwenGeneration: 'q' }, verdict: 'blocked' };
