...
function configureTickets() {
  ...
  const reconciler = createTicketReconciler({ client, identity, log: (event) => log.append(event), onTicket: worker ? (ticket) => worker.run(ticket) : undefined, onReceipt, onMaintenance: async () => { try { await worker?.recover(); await cleanup.resumePending(); } catch (error) { log.append({ event: 'cleanup_resume_failed', error: error.message }); } } });
