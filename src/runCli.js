...
export async function runCommand(commandName, manifestFile) {
  ...
  if (commandName === 'start') { const check = preflightManifest(manifestFile); if (!check.ok) return { ...check, command: commandName }; }
...
if (fileURLToPath(import.meta.url) === process.argv[1]) {
  try { const result = await runCommand(command, value('--manifest')); console.log(JSON.stringify(result)); if (result.ok === false) process.exitCode = 2; }
