export function createTicketNotifier({ peerRequestClient, log = () => {} } = {}) {
  ...
  return { async notify({ tunnelKey, ticketId, targetMachineKey, targetProjectKey, correlationId }) {
    ...
    const result = await peerRequestClient.send(tunnelKey, prompt); log({ event: 'notification_accepted', ticketId, requestId: result?.requestId || null }); return result;
  } };
}
