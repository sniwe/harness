...
export function createServiceController({ file, spawnImpl = spawn, probe = (pid) => { try { process.kill(pid, 0); return true; } catch { return false; } } } = {}) {
  ...
  function inspect() { const state = read(); if (!state) return { state: 'missing' }; if (state.state === 'running' && !probe(state.pid)) return write({ ...state, state: 'unknown', reason: 'owned_process_missing' }); if (state.state === 'stopping' && !probe(state.pid)) return write({ ...state, state: 'stopped', stoppedAt: state.stoppedAt || new Date().toISOString() }); return state; }
