...
export function createServiceController({ file, spawnImpl = spawn, probe = (pid) => { try { process.kill(pid, 0); return true; } catch { return false; } }, now = Date.now, supervisor = createProcessSupervisor() } = {}) {
  ...
  function stop() { const state = inspect(); if (state.state !== 'running') return state; try { supervisor.terminate(state.pid); } catch {} return write({ ...state, state: 'stopping', stoppedAt: new Date().toISOString() }); }
