...
export function createOperationOutbox(root) {
  ...
  const fileFor = (operationId) => { if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(operationId || '')) throw new Error('operation_id_invalid'); return path.join(directory, `${operationId}.json`); };
  ...
  function intent(operationId, operation) { const file = fileFor(operationId); if (fs.existsSync(file)) return JSON.parse(fs.readFileSync(file, 'utf8')); const record = { schemaVersion: 1, operationId, status: 'intent', ...operation }; write(file, record); return record; }
