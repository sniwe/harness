...
export function validateCheckoutContract(expected, observed) {
  const mismatches = [];
  if (expected?.origin && expected.origin !== observed?.checkout?.origin) mismatches.push({ field: 'origin', expected: expected.origin, observed: observed?.checkout?.origin || null });
  if (expected?.branch && expected.branch !== observed?.checkout?.branch) mismatches.push({ field: 'branch', expected: expected.branch, observed: observed?.checkout?.branch || null });
  return mismatches;
}
...
export async function preflightManifest(file, { projectConfig = path.resolve('config/tickets.projects.json'), waitForReady = false, signal, pollMs = 1000, sleep } = {}) {
  try {
    ...
    const checkoutMissing = Object.values(checkouts).filter((checkout) => checkout.state === 'blocked'); for (const [projectKey, checkout] of Object.entries(checkouts)) if (checkout.state === 'observed') checkout.contractMismatches = validateCheckoutContract(manifest.projects[projectKey], checkout); const checkoutMismatches = Object.values(checkouts).filter((checkout) => checkout.contractMismatches?.length); return { ok: missing.length === 0 && checkoutMissing.length === 0 && checkoutMismatches.length === 0 && adapterMissing.length === 0 && runtimeMissing.length === 0 && contractReady && commands.ok, state: missing.length || checkoutMissing.length || checkoutMismatches.length || adapterMissing.length || runtimeMissing.length || !contractReady || !commands.ok ? 'blocked' : 'ready', manifest: { runId: manifest.runId, planDigest: manifest.planDigest }, projects, missing, instructions, checkouts, adapters, runtimes, contractReady, commands };
