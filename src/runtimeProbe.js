export async function probeRuntime({ baseUrl, requiredPath, required = {}, fetchImpl = fetch, signal } = {}) {
  if (!baseUrl || !requiredPath) return { state: 'blocked', error: 'runtime_probe_config_invalid' };
  try { const response = await fetchImpl(`${String(baseUrl).replace(/\/+$/, '')}${requiredPath}`, signal ? { signal } : {}); const body = await response.json().catch(() => null); if (!response.ok || !body || body.ok !== true || Object.entries(required).some(([key, value]) => keyPath(body, key) !== value)) return { state: 'blocked', error: `runtime_capability_mismatch`, status: response.status }; return { state: 'ready', status: response.status, body }; }
  catch (error) { return { state: 'blocked', error: error.name === 'AbortError' ? 'runtime_probe_cancelled' : 'runtime_probe_failed' }; }
}
