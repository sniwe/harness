...
const planDigest = JSON.parse(fs.readFileSync('C:\\harness\\config\\runs\\audep-speed.json', 'utf8')).planDigest;
const verifierProfile = value('--verifier-profile');
const appRoot = 'C:\\retry-harness-run';
const qwenRoot = 'C:\\Users\\rhyse\\Qwen3-ASR';
...
if (!checks[stepId]) { console.error(`phase_command_unknown:${stepId}`); process.exitCode = 2; }
  ...
  const child = spawn(command, args, { cwd, env: { ...process.env, AUDEP_APP_URL: process.env.AUDEP_APP_URL || 'http://127.0.0.1:3102', AUDEP_987_SOURCE: `${appRoot}\\src\\backend\\data\\media\\1788683394546-ebc52030-3330-4ddb-92dc-b95d9c76a26d-987-mtmo3aqe-c4b8b6ddfda248.mp3` }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
  let output = '';
