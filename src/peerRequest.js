...
export function validateRemotePromptEnvelope(payload, { targetKey, maxPromptChars = MAX_PROMPT_CHARS } = {}) {
  ...
  const allowed = new Set(["requestId", "targetTunnelKey", "prompt"]);
...
async function sendPeerRequest({ peerPing, peerKey, prompt, callerKey, fetchImpl, pollMs, sleep, requestId, signal }) {
  ...
  while (true) {
    ...
    try {
      ...
      if (body.state === "worker_failed") throw peerRequestError(502, body.error || body.state, { requestId: id, state: body.state });
    } catch (error) {
      if (error.status === 400 || error.message === "worker_failed") throw error;
    }
  }
}
