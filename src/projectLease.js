...
export function acquireProjectLease(root, { projectKey, pid = process.pid, now = Date.now(), isAlive = (ownerPid) => { try { process.kill(ownerPid, 0); return true; } catch { return false; } } } = {}) {
  ...
  try { const fd = fs.openSync(file, 'wx'); fs.writeSync(fd, JSON.stringify(lease)); fs.closeSync(fd); } catch (error) { if (error.code !== 'EEXIST') throw error; let owner; try { owner = JSON.parse(fs.readFileSync(file, 'utf8')); } catch { owner = null; } if (!owner || !isAlive(Number(owner.pid))) { try { fs.unlinkSync(file); } catch {} return acquireProjectLease(root, { projectKey, pid, now, isAlive }); } throw new Error('project_lease_busy'); }
