export function createTicketClient({ baseUrl = process.env.TICKETS_BASE_URL, fetchFn = globalThis.fetch, timeoutMs = 30000 } = {}) {
  ...
  async function request(method, route, payload) {
    ...
    try {
      ...
      if (!result.ok) { const error = new Error(body.code || `ticket_http_${result.status}`); error.status = result.status; error.body = body; throw error; }
      return body;
