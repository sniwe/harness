...
export function evaluateFinalAcceptance({ app, qwen, restarts = [], minimumRealtime = 0.5 } = {}) {
  ...
  if (!Number.isFinite(app.metrics.committedRealtime) || app.metrics.committedRealtime < minimumRealtime) return { ok: false, verdict: 'failed', reason: 'throughput_below_minimum', observed: app.metrics.committedRealtime, minimumRealtime, joined };
