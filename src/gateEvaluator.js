import { validateGateEvidence } from './audepGateCatalog.js';
...
export function evaluateEvidence({ evidence, step, manifest }) {
  ...
  if (manifest.protocolBaseline === '12.5s-primary-plus-5s-boundary-v1') {
    try { const gate = validateGateEvidence({ step, evidence }); if (!gate.ok) return gate; } catch (error) { if (!error.message.startsWith('audep_gate_unknown:')) throw error; }
  }
