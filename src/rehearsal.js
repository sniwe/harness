...
export function renderRunReport({ manifest, result }) {
  ...
  for (const step of Object.values(result.state.steps)) { const manifestStep = manifest.steps.find((item) => item.stepId === step.stepId); const waiting = manifestStep?.dependsOn?.filter((id) => result.state.steps[id]?.status !== 'succeeded') || []; lines.push(`- ${step.stepId}: ${step.status}${step.gateId ? ` (${step.gateId})` : ''}${step.owner ? `; owner ${step.owner}` : ''}${step.lastHeartbeatAt ? `; heartbeat ${step.lastHeartbeatAt}` : ''}${waiting.length ? `; waiting on ${waiting.join(', ')}` : ''}`); }
