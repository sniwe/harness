...
export function renderRunReport({ manifest, result }) {
  ...
  if (result.finalAcceptance) { lines.push('', '## Final acceptance', '', `- Verdict: **${result.finalAcceptance.ok ? 'PASS' : String(result.finalAcceptance.verdict || 'BLOCKED').toUpperCase()}**`); if (result.finalAcceptance.reason) lines.push(`- Reason: \`${result.finalAcceptance.reason}\``); if (result.finalAcceptance.missing?.length) lines.push(`- Missing restart evidence: ${result.finalAcceptance.missing.join(', ')}`); for (const [project, cohort] of Object.entries(result.finalAcceptance.warmCohort || {})) if (cohort) lines.push(`- ${project} warm cohort: ${cohort.observedRuns || 0} runs; median \`${cohort.medianRealtime ?? 'n/a'}x\`; range \`${cohort.range?.minimum ?? 'n/a'}–${cohort.range?.maximum ?? 'n/a'}x\``); }
