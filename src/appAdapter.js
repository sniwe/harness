...
export function resolveAppBenchmarkAdapter({ projectRoot, verifier, source, appUrl = 'http://127.0.0.1:3000', sourceAudioRevision, durationMs, ffprobePath = process.env.FFPROBE_PATH || 'ffprobe' } = {}) {
  ...
  if (durationMs !== undefined) { try { measuredDurationMs = Math.floor(Number(execFileSync(ffprobePath, ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', sourcePath], { encoding: 'utf8' }).trim()) * 1000); } catch { throw new Error('app_987_duration_probe_failed'); } if (!Number.isFinite(measuredDurationMs) || measuredDurationMs !== durationMs) throw new Error(`app_987_duration_mismatch:expected=${durationMs}:measured=${measuredDurationMs}`); }
