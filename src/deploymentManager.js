...
export function createDeploymentManager(file) {
  ...
  async function waitForHealthy({ observe, sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), pollMs = 1000, signal } = {}) {
    ...
    while (true) {
      ...
      if (observed?.state === 'healthy' && observed.runtimeGeneration && observed.commit) {
        try { return markHealthy({ runtimeGeneration: observed.runtimeGeneration, commit: observed.commit }); }
        catch (error) { if (error.message !== 'deployment_commit_mismatch') throw error; write({ ...read(), lastHealthObservation: { state: observed.state, runtimeGeneration: observed.runtimeGeneration, commit: observed.commit, error: error.message, observedAt: new Date().toISOString() } }); }
      }
      await sleep(pollMs);
    }
  }
