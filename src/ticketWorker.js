...
export function createTicketWorker({ client, pool, identity, log, lockRoot, journal, projectRoot = process.env.MACHINE_BASE_RUNTIME_CWD || process.cwd(), attemptStore, operationOutbox, leaseRoot }) {
  ...
  async function run(ticket) {
    ...
    try {
      ...
      const startOperationId = `start:${ticket.ticketId}`; await mutate(ticket.ticketId, startOperationId, { operationId: startOperationId, expectedRevision: claimed.ticket.revision, previousHash: claimed.ticket.headHash, actor, action: 'start', data: { leaseToken } }); log({ event: 'execution_started', ticketId: ticket.ticketId, executionId: `ticket:${ticket.ticketId}` });
      started = true;
      const result = requireWorkerSuccess(await pool.request({ task: 'remote-prompt', requestId: `ticket:${ticket.ticketId}`, prompt: ticketPrompt(ticket) })); const outcomeBytes = Buffer.from(JSON.stringify(result), 'utf8'); const outcomeHash = crypto.createHash('sha256').update(outcomeBytes).digest('hex'); journal?.append({ event: 'execution_outcome', ticketId: ticket.ticketId, executionId: `ticket:${ticket.ticketId}`, outcomeHash }); attemptStore?.finish(attempt?.attemptId, { state: 'succeeded', outcomeHash, outcome: result });
      const receiptTicketId = crypto.createHash('sha256').update(`receipt/${ticket.ticketId}`).digest('hex');
      const receiptOperationId = `receipt:${ticket.ticketId}`; await create(receiptOperationId, { ticketId: receiptTicketId, operationId: receiptOperationId, kind: 'receipt', sender: actor, target: ticket.sender, conversationId: ticket.conversationId, correlationId: ticket.correlationId, parentTicketId: ticket.ticketId, testRunId: ticket.testRunId, subject: `Receipt ${ticket.ticketId}`, body: 'Ticket completed.', sourceTicketId: ticket.ticketId, resultCode: 'success', resultSummary: 'Worker completed.', outcomeHash });
