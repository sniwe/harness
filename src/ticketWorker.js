...
export function createTicketWorker({ client, pool, identity, log, lockRoot, journal, projectRoot = process.env.MACHINE_BASE_RUNTIME_CWD || process.cwd(), attemptStore, operationOutbox, leaseRoot, leaseRenewMs = 30000, now = Date.now, isAlive = (ownerPid) => { try { process.kill(ownerPid, 0); return true; } catch { return false; } } }) {
  ...
  async function run(ticket) {
    ...
    let started = false; let outcomePersisted = false; let leaseToken; let projectLease; let leaseTimer; let attempt; const actor = identity;
    ...
    try {
      ...
      leaseTimer = projectLease && leaseRenewMs > 0 ? setInterval(() => { try { projectLease.renew(now()); } catch (error) { log({ event: 'project_lease_renew_failed', ticketId: ticket.ticketId, error: error.message }); } }, leaseRenewMs) : undefined;
      const ensureLease = () => projectLease?.assertOwned();
      ...
      ensureLease(); const claimOperationId = `claim:${ticket.ticketId}`; const claimed = await mutate(ticket.ticketId, claimOperationId, { operationId: claimOperationId, expectedRevision: head.revision, previousHash: head.headHash, actor, action: 'claim', data: { leaseToken } });
      ensureLease(); const startOperationId = `start:${ticket.ticketId}`; await mutate(ticket.ticketId, startOperationId, { operationId: startOperationId, expectedRevision: claimed.ticket.revision, previousHash: claimed.ticket.headHash, actor, action: 'start', data: { leaseToken } }); log({ event: 'execution_started', ticketId: ticket.ticketId, executionId: `ticket:${ticket.ticketId}` }); started = true;
      ensureLease(); const result = requireWorkerSuccess(await pool.request({ task: 'remote-prompt', requestId: `ticket:${ticket.ticketId}`, prompt: ticketPrompt(ticket) }), { expectedCwd: projectRoot, expectedProjectKey: identity.projectKey }); const outcomeHash = crypto.createHash('sha256').update(JSON.stringify(result)).digest('hex'); journal?.append({ event: 'execution_outcome', ticketId: ticket.ticketId, executionId: `ticket:${ticket.ticketId}`, outcomeHash }); attemptStore?.finish(attempt?.attemptId, { state: 'succeeded', outcomeHash, outcome: result }); outcomePersisted = true;
      ensureLease(); const receiptTicketId = crypto.createHash('sha256').update(`receipt/${ticket.ticketId}`).digest('hex'); const receiptOperationId = `receipt:${ticket.ticketId}`; await create(receiptOperationId, { ticketId: receiptTicketId, operationId: receiptOperationId, kind: 'receipt', sender: actor, target: ticket.sender, conversationId: ticket.conversationId, correlationId: ticket.correlationId, parentTicketId: ticket.ticketId, testRunId: ticket.testRunId, subject: `Receipt ${ticket.ticketId}`, body: 'Ticket completed.', sourceTicketId: ticket.ticketId, resultCode: 'success', resultSummary: 'Worker completed.', outcomeHash });
      ensureLease(); const done = (await client.get(ticket.ticketId)).ticket; const completeOperationId = `complete:${ticket.ticketId}`; const final = await mutate(ticket.ticketId, completeOperationId, { operationId: completeOperationId, expectedRevision: done.revision, previousHash: done.headHash, actor, action: 'complete', data: { leaseToken, receiptTicketId, outcomeHash } }); log({ event: 'work_completed', ticketId: ticket.ticketId, executionId: `ticket:${ticket.ticketId}` }); return final;
    } catch (error) { if (!outcomePersisted) { try { attemptStore?.finish(attempt?.attemptId, { state: 'failed', error: error.message }); } catch {} } log({ event: outcomePersisted ? 'execution_outcome_publication_failed' : 'execution_failed', ticketId: ticket.ticketId, error: error.message }); if (started && !outcomePersisted) { try { const current = await client.get(ticket.ticketId); if (['claimed', 'in_progress'].includes(current.ticket.status)) { const blockOperationId = `block:${ticket.ticketId}`; await mutate(ticket.ticketId, blockOperationId, { operationId: blockOperationId, expectedRevision: current.ticket.revision, previousHash: current.ticket.headHash, actor, action: 'block', data: { reason: 'unknown_after_crash', leaseToken } }); } } catch (blockError) { log({ event: 'block_failed', ticketId: ticket.ticketId, error: blockError.message }); } } throw error;
    } finally {
